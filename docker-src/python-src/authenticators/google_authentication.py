...
async def handle_credentials(request):
	...
	# The primary key for this authenticator is the email address of the account
...
# This function will be used to authorize users to use
# the web app. The primary key is what authorizes them
# The below function gets passed whatever is in the id cookie
# (without the namespace, eg. just 1234 instead of google:1234)
async def get_primary_key(db, uuid):
	try:
		return (await db.get(f"cookies:{uuid}")).decode()
	except AttributeError:
		# Return falsy value if the user isn't authenticated
		return ""
